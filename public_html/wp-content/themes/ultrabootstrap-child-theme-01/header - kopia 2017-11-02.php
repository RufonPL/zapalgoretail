...
<head>
	...
	<!-- <script src="http://www.retailer.e-conex.pl/wp-content/themes/ultrabootstrap-child-theme-01/jquery.jscroll.min.js"></script>  -->
...
div#s2id_billing_country > a > span.select2-arrow > b, div#s2id_shipping_country > a > span.select2-arrow > b {
    display: none !important;
}
...
a.tm-cart-link.tc-poplink:before {
    content: 'See details';
    position: relative;
    font-size: 14px;
}
...
$(document).ready(function(){
	$("input#billing_first_name").attr("required", "true");
	$("input#billing_last_name").attr("required", "true");
	$("input#billing_email").attr("required", "true");
	$("input#billing_phone").attr("required", "true");

	$("input.button.alt.yith-wcms-button.next.button-step-1").click(function() {
	    if( !$('input#billing_first_name').val() || !$('input#billing_last_name').val() || !$('input#billing_email').val() || !$('input#billing_phone').val() ) {
	    	$('.woocommerce-error').after('<ul class="woocommerce-error"><li class="terms-error">You must complete all the empty fields</li></ul>');
	    }
	});
...
$(window).mousemove(function() {
	...
	if(numItems == 1) {
		$("li.tmcp-field-wrap").click(function() {
	...
	if(numItems == 5) {
		$(".tm-extra-product-options-container > ul > li").click(function() {
			$(this).addClass('tc-active');
...
$(document).ready(function() { 
	...
	$('#ChangeCurrency').click(function(){
		jQuery.removeCookie('aelia_cs_selected_currency', { path: '' });
		jQuery.removeCookie('customer_default_country', { path: '' });
		jQuery.removeCookie('aelia_customer_country', { path: '' });
		setTimeout( function() {
			location.reload();
		}, 500);
	});
...
$(window).load(function() { 
	if (jQuery.cookie('aelia_billing_country') == 'HR' ){
    ...
    <nav class="navbar navbar-default navbar-fixed-top up-down" role="navigation">
      ...
      <div class="navbar-collapse collapse">
        ...
        <?php if ( is_user_logged_in() ) { ?>
			<li class="navbar-left"><a href="<?php echo wp_logout_url(); ?>/session/">Sign out</a></li>
...
<div class=" hidden-xs">
    <nav class="navbar navbar-default navbar-fixed-top up-down " role="navigation">
      ...
      <div class="navbar-collapse collapse">
        ...
        <?php if ( is_user_logged_in() ) { ?>
			<li class="navbar-left"><a href="<?php echo bloginfo('url'); /*wp_logout_url();*/ ?>/session/">Sign out</a></li>
			<li class="navbar-left"><a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('My Account','woothemes'); ?>"><?php _e('My Account','woothemes'); ?></a></li>
		<?php } else { ?>
		  <li class="navbar-left dropdown">
		    <a class="dropdown-toggle" data-toggle="dropdown" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Login / Register','woothemes'); ?>"><?php _e('Sign in','woothemes'); ?>
		...
		<?php } ?>
          ...
          <li class="navbar-right dropdown">
          	<a id="ChangeCurrency" class="dropdown-toggle" href="<?php bloginfo('url'); ?>/zapalgo/cart/" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			    EUR-USD
		  	</a>
          </li>
